 ...
 *
 * Takes the menu as a prop so the same card renders the house menu and
 * any one-night variant of it — one layout, no second copy to keep in
 * step when the design changes.
 */
        ...
        <div className="rounded-md bg-white px-6 py-10 text-center shadow-2xl sm:px-12">
          ...
          <h1 className="h-sign text-4xl text-teal sm:text-5xl">
            ...
            <sup className="text-2xl sm:text-3xl">
              {menu.sections[0].price}
            </sup>
          </h1>

          <div className="mt-8 grid gap-8 sm:grid-cols-2">
            {menu.sections[0].drinks.map((d) => (
              <div key={d.name}>
                {d.image && (
          ...
          <h2 className="h-sign mt-8 text-4xl text-teal sm:text-5xl">
            {menu.sections[1].title}{" "}
            <sup className="text-2xl sm:text-3xl">
              {menu.sections[1].price}
            </sup>
          </h2>

          {menu.sections[1].drinks.map((d) => (
          ...
          {menu.addOn && (
            <p className="h-sign-med mt-6 text-lg text-teal">
              ...
              <span className="underline underline-offset-4">
                {menu.addOn.highlight}
              </span>{" "}
              to any drink!{" "}
              <span className="text-abyss">{menu.addOn.price}</span>
            </p>
          )}
        </div>
